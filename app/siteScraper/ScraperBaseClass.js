...
class ScraperBaseClass {
  ...
  */
  ...
  async getProperties() {
    const properties = [];
    let propertyPage = await this.getNextPropertyPage();
    while (propertyPage) {
      const dom = await this.getDom(propertyPage);
      if (dom) {
        const domProperties = Array.from(dom.window.document.querySelectorAll(this.properySelector));
        domProperties.forEach((propertyDom) => {
          const property = this.getPropertyFromDom(propertyDom);
          properties.push(property);
        });
      }
      propertyPage = await this.getNextPropertyPage();
    }
    return properties;
